...
public class ArrayDeque<T> {
    ...
    public void addFirst(T data) {
        ...
        checkForResize("first");
        int firstIndex = mod(front - 1, backingArray.length);
        backingArray[firstIndex] = data;
        front--;
        if (front < 0) {
    ...
    public void addLast(T data) {
        ...
        checkForResize("last");
        int lastIndex = mod(front + size, backingArray.length);
        backingArray[lastIndex] = data;
        size++;
    }

    /**
     * Checks to resize the array depending on how you add an element.
     * This is an O(n) operation.
     * @param keyWord Either the words "first" or "last" can be passed in.
     */
    private void checkForResize(String keyWord) {
        if (size < backingArray.length) {
            return;
        }
        T[] unWrappedArray = backingArray;
        backingArray = (T[]) new Object[size * 2];
        for (int i = 0; i < size; i++) {
            int circularIndex = mod(front + i, size);
            if (keyWord.equals("first")) {
                backingArray[i + 1] = unWrappedArray[circularIndex];
            } else {
                backingArray[i] = unWrappedArray[circularIndex];
            }
        }
        front = 0;
        if (keyWord.equals("first")) {
    ...
    public T removeFirst() {
        ...
        int firstIndex = mod(front, backingArray.length);
        T removed = backingArray[firstIndex];
    ...
    public T removeLast() {
        ...
        int lastIndex = mod(front + size - 1, backingArray.length);
    ...
    public T getLast() {
        ...
        return backingArray[mod(index, backingArray.length)];
    }
